...
limits2<-aes(ymax=c(new3S[which(new3S$RCP=="RCP45"),]$med.diff+new3S[which(new3S$RCP=="RCP45"),]$sd),ymin=c(new3S[which(new3S$RCP=="RCP85"),]$med.diff-new3S[which(new3S$RCP=="RCP85"),]$sd))
labels<-c("Sagebrush","Other shrubs","C3PG","C4PG","C3AG","PF","AF")
datagg2<-data.frame(x=rep(1.5,7),y=rep(50,7),label=labels,Rgroup=unique(new3S$Rgroup))
BMASS.br<-ggplot(new3S[which(new3S$RCP=="RCP45"),], 
...
source("code_figures/STEPWAT_functional.composition.RCP45.21.10.2022.R")
library(patchwork)

combined<-BMASS.br+Composition_plot+ plot_layout(guides = "collect") & theme(legend.position = "bottom")

pdf(height=8,width=15,paste0("figures/RCP45_comp_bar.pdf"))
combined
dev.off()
